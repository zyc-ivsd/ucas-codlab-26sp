...
int main() {
  ...
  for (int i = 0; i < ARR_SIZE(benchmarks); i ++) {
    ...
    if (msg != NULL) {
      ...
      unsigned long mem_cycles = ULONG_MAX;
      unsigned long retired_cycles = ULONG_MAX;
      int succ = 1;
      for (int i = 0; i < REPEAT; i ++) {
        ...
        if (res.cycles < cycles) cycles = res.cycles;
        if (res.mem_cycles < mem_cycles) mem_cycles = res.mem_cycles;
        if (res.retired_cycles < retired_cycles) retired_cycles = res.retired_cycles;
      }
      ...
      printk(" cycles = %u, mem_access = %u, inst_retired = %u\n", cycles, mem_cycles, retired_cycles);
    }
  }
